...
function closeGalleryViewer() { document.getElementById("galleryViewer").style.display = 'none'; document.getElementById("galleryViewerImg").src = ''; document.getElementById("galleryViewerImg").className = ''; document.getElementById("galleryViewerImg").style.transform = ''; const vw = document.querySelector('.gallery-video-wrap'); if (vw) { vw.remove(); } viewerZoom = 1; viewerIndex = -1; viewerDrag.active = false; }
...
function setViewerTransform(tx, ty) { const img = document.getElementById("galleryViewerImg"); const zoomed = viewerZoom > 1; img.className = zoomed ? 'zoomed' : ''; if (zoomed) { img.style.transform = `translate(${tx}px, ${ty}px) scale(${viewerZoom})`; } else { img.style.transform = ''; } }
